...
export function canonicalizeUsage(usage) {
  ...
  // OptiAI: upstream dropped this flag here, so every persisted row looked provider-reported
  // even when estimateUsage() produced it from a chars/4 heuristic. OptiAI's analyzer must not
  // present a guess as a measurement, so the provenance rides along into the `tokens` JSON column.
  if (usage.estimated) result.estimated = true;
  return result;
}
